import { Orkinism } from '../classes/orkinism';

//word, type, phonetic, definition
export const orkinisms: Orkinism[] = [
    new Orkinism(
        'tookie; wookie; tysie; tooksie; pug; puggles; puglet; the pug; pugloot; little face; handsome; bobbinhead; zootie',
        'noun',
        '[tuuk-ee]',
        'The family dog/prince/icon, Tyson Orkin.'
    ),
    new Orkinism(
        'shink; shinkies; shinkalink; shinkadink; shinkalinky',
        'noun',
        '[shingk]',
        'Shannon Orkin.'
    ),
    new Orkinism(
        'taps; tips; tapsy; tipson',
        'noun',
        '[taps]',
        'Taryn Orkin.'
    ),
    new Orkinism(
        'noonkie',
        'noun',
        '[noon-kee]',
        'I\'m not even sure anymore.'
    ),
    new Orkinism(
        'twinkie',
        'noun',
        '[twin-kee]',
        'A twinkie.'
    ),
    new Orkinism(
        'Master Jack',
        'noun',
        '[mah-ster jak]',
        'Gran\'s boyfriend name placeholder.'
    ),
    new Orkinism(
        'tookalook; tookalooks',
        'noun',
        '[took-A-look]',
        'How Chloe refers to one of the clan (can be any of the siblings).'
    ),
    new Orkinism(
        'shamin',
        'noun',
        '[sham-in]',
        'When Hilton forgets all of the kids names and needs to call them something.'
    ),
    new Orkinism(
        'hamin',
        'noun',
        '[ham-in]',
        'When Hilton really gets confused (refer shamin).'
    ),
    new Orkinism(
        'doodle',
        'noun',
        '[dood-l]',
        'If someone is being silly; a poop (refer niggie).'
    ),
    new Orkinism(
        'niggie',
        'noun',
        '[nig-gee]',
        'When the pug poops, the last person to yell this out has to pick up the poo.'
    ),
    new Orkinism(
        'rookie',
        'noun',
        '[rook-ee]',
        'A misspelling of tookie/wookie. Also a televsion show.'
    ),
    new Orkinism(
        '👍🏼',
        'emoji',
        '[N/A]',
        'Hilton\'s standard emoji response, for the affirmative.'
    ),
    new Orkinism(
        '🖕🏼',
        'emoji',
        '[N/A]',
        'Hilton\'s standard emoji response, for the defensive.'
    ),
    new Orkinism(
        'hoolamachala',
        'verb',
        '[hoo-la-mach-uh-lah]',
        'Hurry up - get on your bicycles!'
    ),
    new Orkinism(
        'kom eetem de spraken',
        'phrase',
        '[N/A]',
        'Come eat! (Afrikaans)'
    ),
    new Orkinism(
        'padoonkie',
        'noun',
        '[pad-oon-ee]',
        'A poop, typically human.'
    ),
    new Orkinism(
        '...chicken scratch...',
        'sound',
        '[N/A]',
        'Undefinable, more of a noise. Used to alert other and Tysie to the fact he is sniffing around in bushes, or scratching himself.'
    ),
    new Orkinism(
        '"YOU {INSERT WORD/PHRASE}"',
        'phrase',
        '[N/A]',
        'Used to fire something back at someone (example: "Are you ready?" No, YOU ready!)'
    ),
    new Orkinism(
        'doodoo; kip',
        'noun',
        '[doo-doo]',
        'A afternoon nap.'
    ),
    new Orkinism(
        'dingleberry',
        'noun',
        '[ding-guhl-ber-ee]',
        'When Tysie has poo stuck to his bum; '
    ),
    new Orkinism(
        'fuck-knuckle',
        'noun',
        '[fuhk-nuhk-uhl]',
        'Chloe\'s peak level of affection for Hilton.'
    ),
    new Orkinism(
        'dingbat',
        'noun',
        '[ding-bat]',
        'A detestable term for Hilton.'
    ),
    new Orkinism(
        'milton; biltong; eaton; pigeon; guilton; elton; hamilton; elton; clinton',
        'noun',
        '[N/A]',
        'Hilton Orkin\'s various names (at the altar, coffee shops, etc).'
    ),
    new Orkinism(
        'W',
        'letter',
        '[duhb-uhl-yoo]',
        'When you can\'t say the the work \'walk\' in front of Tysie.'
    ),
    new Orkinism(
        '"Dramatic pug"; "Drama llama"',
        'phrase',
        '[druh-mat-ik puhg]',
        'When someone is being dramatic.'
    ),
    new Orkinism(
        'squish',
        'noun',
        '[skwish]',
        'When someone is being cute, perhaps overly so.'
    ),
    new Orkinism(
        'NOIIIICCEEE',
        'adjective',
        '[nooooois]',
        'Historically, appreciation of a Christmas present.'
    ),
    new Orkinism(
        '"Swipe swipe, swipe swipe"',
        'phrase/sing-a-long',
        '[swahyp swahyp, swahyp swahyp]',
        'Appreciation of a Christmas present; Hilton\'s contribution to said present.'
    ),
    new Orkinism(
        '"See you on the Christmas tree"',
        'phrase',
        '[N/A]',
        'Something Gran says (undefinable).'
    ),
    new Orkinism(
        'funny-face',
        'noun',
        '[fuhn-ee feys]',
        'When Gran reckons you have a funny face.'
    ),
    new Orkinism(
        'fat-face',
        'noun',
        '[fat-feys]',
        'When Gran reckons you have a fat face/are scratching too much in the cupboards.'
    ),
    new Orkinism(
        'weenie',
        'noun',
        '[ween-ee]',
        'Scaredy-cat (do NOT say to Chloe Orkin).'
    ),
    new Orkinism(
        'dookie',
        'noun',
        '[dook-ee]',
        'Anything that sticks out a little; the little bump of the back of Ryan\'s head.'
    ),
    new Orkinism(
        'longnecks',
        'noun',
        '[lawng-nek]',
        'Giraffes (not to be confused with baby giraffes).'
    ),
    new Orkinism(
        '"What\'s that screaming?"',
        'phrase',
        '[hwuhts that skree-ming]',
        'Uttered whenever Hilton is playing his music.'
    ),
    new Orkinism(
        'just quickly',
        'phrase',
        '[juhst kwik-lee]',
        'Slowly.'
    ),
    new Orkinism(
        '"Duck walk"',
        'phrase',
        '[duhk wawk]',
        'When Tappy Time has concluded and time must be made up.'
    ),
    new Orkinism(
        'little',
        'adjective',
        '[lit-l]',
        'Can be used to refer to anything from tiny to huge (example: A tiny coffee could be a quick takeaway or a long sit-down lunch).'
    ),
    new Orkinism(
        '"Tappy time"',
        'phrase',
        '[tap-ee tahym]',
        'When someone is late (refer tappy).'
    ),
    new Orkinism(
        '"The red thing"; "Red rocket"',
        'phrase',
        '[thee red thing]',
        'Tysie\'s doodle (refer doodle).'
    ),
    new Orkinism(
        'mommying',
        'noun',
        '[mom-ee-ing]',
        'Sleeping on your back and not moving - mostly for Hilton.'
    ),
    new Orkinism(
        '"Na brah"',
        'phrase',
        '[nah brah]',
        'No.'
    ),
    new Orkinism(
        'goggles; goggaloos',
        'noun',
        '[gog-uhls]',
        'What Chloe refers to as glasses.'
    ),
    new Orkinism(
        '"Thanks Mum!"',
        'phrase',
        '[thangks muhm]',
        'Appreciation for something done by Hilton.'
    ),

];



// 
// Shinkies
// Shinkalink
// Shinks